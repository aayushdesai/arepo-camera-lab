...
let rulerPoints=[],rulerKind=null,rulerPicking=false;
const meshLive=Boolean(DATA.scene.native_mesh_available)&&location.protocol.startsWith('http');
renderMode.value=meshLive?(localStorage.getItem('arepo_camera_lab_renderer_v001')||'mesh'):'points';
if(!meshLive)renderMode.querySelector('[value="mesh"]').disabled=true;
meshNotice.textContent=meshLive?'Full native cell geometry; the physical range controls which cells are visible.':'Native mesh mode needs the live server and a scene with native connectivity.';
...
function meshParameters(){
  const aspect=canvas.width/canvas.height,limit=dragging?560:1440;
  ...
  return {scene_sha256:DATA.scene.sha256,camera:{target:[...camera.target],forward:[...camera.forward],up:[...camera.up],scale:camera.scale},style:meshStyle(),derived_channels:derivedDefinitions,edges:meshEdges.checked,interior_faces:meshInterior.checked,lighting:meshLighting.checked,density_floor:+meshDensityFloor.value,fit_visible:meshFitRequested,width,height};
}
function meshKey(params){return JSON.stringify(params);}
async function requestNativeFrame(force=false){
  if(renderMode.value!=='mesh')return;
  const params=meshParameters(),key=meshKey(params);
  ...
  meshNotice.textContent=meshLastReport?'Updating the 3D cell view…':'Loading the native 3D cell view…';
  meshPromise=(async()=>{
    try{
      ...
      if(sequence!==meshRequestSequence||renderMode.value!=='mesh')return;
      ...
      const r=result.report;meshNotice.textContent=`${r.selected_cells.toLocaleString()} / ${r.native_cell_count.toLocaleString()} cells in range · ${r.faces.toLocaleString()} native faces · ${r.render_seconds.toFixed(2)} s`;
...
renderMode.onchange=()=>{localStorage.setItem('arepo_camera_lab_renderer_v001',renderMode.value);meshImage.style.display=renderMode.value==='mesh'&&meshLastKey?'block':'none';rulerPoints=[];rulerKind=null;meshFailureKey=null;};
...
document.getElementById('clearRuler').onclick=()=>{rulerPoints=[];rulerKind=null;updateMeasurements();};
...
function updateMeasurements(){
  ...
  const rect=canvas.getBoundingClientRect(),shown=renderMode.value==='mesh'&&meshDisplayedCamera?meshDisplayedCamera:camera,aspect=canvas.width/canvas.height;
  ...
  const style=renderMode.value==='mesh'&&meshLastReport?meshLastReport.style:meshStyle(),legendKey=JSON.stringify(style),meta=DATA.channels[style.channel]??currentChannel;
...
async function rulerPick(event){
  ...
  try{
    ...
    if(renderMode.value==='mesh'){
      if(meshBusy||meshLastKey!==meshKey(meshParameters()))throw new Error('Wait for the current mesh frame before picking.');
    ...
    }else{point={position_cm:CameraMeasurements.pointOnPlane(x,y,camera,canvas.width/canvas.height,DATA.scene.display_radius_cm,DATA.scene.center_cm)};kind='projected';}
...
canvas.onpointerdown=event=>{if(rulerToggle.checked){event.preventDefault();rulerPick(event);return;}originalPointerDown(event);};
canvas.ondblclick=event=>{if(!rulerToggle.checked)originalDoubleClick(event);};
const originalPointerUp=canvas.onpointerup;
canvas.onpointerup=event=>{if(canvas.hasPointerCapture(event.pointerId))originalPointerUp(event);};
window.cameraLabMeasurements=()=>({schema:'arepo_camera_lab_measurements_v001',snapshot:DATA.scene.snapshot,scene_sha256:DATA.scene.sha256,kind:rulerKind,points:cloned(rulerPoints),distance_cm:rulerPoints.length===2?CameraMeasurements.distance(rulerPoints[0].position_cm,rulerPoints[1].position_cm):null});
function meshViewState(){return {schema:'arepo_camera_lab_mesh_view_v001',renderer:renderMode.value,density_floor:+meshDensityFloor.value,edges:meshEdges.checked,interior_faces:meshInterior.checked,lighting:meshLighting.checked,annotations:showAnnotations.checked,measurement_unit:measureUnit.value};}
function applyMeshViewState(state){
  if(!state||state.schema!=='arepo_camera_lab_mesh_view_v001')return;
  renderMode.value=state.renderer==='mesh'&&meshLive?'mesh':'points';
  ...
  meshImage.style.display=renderMode.value==='mesh'&&meshLastKey?'block':'none';
}
for(const control of [renderMode,meshDensityFloor,meshEdges,meshInterior,meshLighting,showAnnotations,measureUnit]){control.addEventListener('input',markStyleDirty);control.addEventListener('change',markStyleDirty);}
