...
vm.runInContext(`
const DATA={scene:{native_mesh_available:true,sha256:'abc',snapshot:31,snapshot_time_seconds:155.0146484375,display_radius_cm:10,center_cm:[100,200,300]},channels:{density:{label:'density',units:'g cm^-3'}}};
...
const derivedDefinitions=[];let dragging=false,requests=0,badHash=false,picks=0;
...
async function fetch(url,options){
  ...
  requests++;
  return {ok:true,json:async()=>({png:'dummy',report:{scene_sha256:badHash?'wrong':DATA.scene.sha256,selected_cells:8,native_cell_count:10,faces:12,render_seconds:.05,camera:params.camera,style:params.style}})};
}
`, context);
...
(async () => {
  await vm.runInContext(`(async()=>{
    assert.equal(renderMode.value,'mesh');
  ...
  console.log('Native controller, stale-frame guard, time/scale legend, and both rulers passed');
